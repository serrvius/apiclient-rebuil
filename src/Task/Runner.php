...
use API\Exception\ServiceException;
use API\Exception\TaskException;
    ...
    public function __construct($config, $name, $action, array $arguments = array())
    {
        if (isset($config['initial_delay'])) {
            if ($config['initial_delay'] < 0) {
                throw new TaskException('Task configuration `initial_delay` must not be negative.');
            }
        ...
        if (isset($config['max_delay'])) {
            if ($config['max_delay'] <= 0) {
                throw new TaskException('Task configuration `max_delay` must be greater than 0.');
            }
        ...
        if (isset($config['factor'])) {
            if ($config['factor'] <= 0) {
                throw new TaskException('Task configuration `factor` must be greater than 0.');
            }
        ...
        if (isset($config['jitter'])) {
            if ($config['jitter'] <= 0) {
                throw new TaskException('Task configuration `jitter` must be greater than 0.');
            }
        ...
        if (isset($config['retries'])) {
            if ($config['retries'] < 0) {
                throw new TaskException('Task configuration `retries` must not be negative.');
            }
            $this->maxAttempts += $config['retries'];
        }

        if (!is_callable($action)) {
            throw new TaskException('Task argument `$action` must be a valid callable.');
        }
